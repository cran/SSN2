cov_initial_search <- function(initial_NA_object, ssn.object, data_object, estmethod) {
  ...
  cov_grid <- data.frame(
    ...
    euclid_range = euclid_range, rotate = 0, scale = 1
  )
  ...
  objvals <- vapply(cov_grid_splits, function(x) eval_grid(x, initial_NA_object, ssn.object, data_object, estmethod), numeric(1))
...
eval_grid <- function(cov_grid_split, initial_NA_object, ssn.object, data_object, estmethod) {
  ...
  # params object
  params_object <- get_params_object_grid(cov_grid, initial_NA_object)
